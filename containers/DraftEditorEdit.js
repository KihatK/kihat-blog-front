...
const DraftEditor = ({ nickname, title, category, language, editing, uuid }) => {
    ...
    useEffect(() => {
        const parsedData = convertFromRaw(JSON.parse(editing));
        setEditorState(EditorState.createWithContent(parsedData));
    }, []);
        ...
        <>
            <StyledDiv>
                ...
                <select onChange={e => toggleColor(e.target.value)}>
                    {options(['green', 'blue', 'red', 'purple', 'orange', 'black', 'white', 'yellow'])}
                </select>
